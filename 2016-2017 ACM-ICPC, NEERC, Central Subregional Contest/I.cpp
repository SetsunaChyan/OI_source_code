...
int main()
{
    ...
    if(m+k<n+2||m+k>n+4) return printf("-1"),0;
    if(m+k==n+2)
        for(int i=0;i<n;i++)
            ans=min(ans,p[i].distTo(p[(i+m-1)%n]));
    else if(m+k==n+3)
        for(int i=0;i<n;i++)
        {
            ans=min(ans,nearest(p[(i+m-1)%n],p[(i+m-2)%n],p[i]));
            ans=min(ans,nearest(p[(i-m+1+n)%n],p[(i-m+2+n)%n],p[i]));
        }
    else
        for(int i=0;i<n;i++)
        {
            ans=min(ans,disSS(p[i],p[(i+1)%n],p[(i+m-2)%n],p[(i+m-1)%n]));
            ans=min(ans,disSS(p[i],p[(i-1+n)%n],p[(i-m+2+n)%n],p[(i-m+1+n)%n]));
        }
    printf("%.10f",ans);
    return 0;
}
