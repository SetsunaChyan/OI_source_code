...
void gen(int k,db x)
{
    pol.clear();
    P t=P(x,0);
    db ang=2.0/k*acos(-1.0);
    for(int i=0;i<k;i++) pol.push_back(t),t=t.rot(ang);
}
...
bool check(int op)
{
    int cnt=0;
    for(int i=0;i<n;i++)
        if(contain(pol,p[i])) cnt++;
    if(op==0) return cnt!=n;
    else return cnt==0;

}

void gao(int k,int op)
{
    db l=1e-3,r=1e8,mid;
    for(int i=0;i<150;i++)
    {
...
int main()
{
    ...
    {
        gao(k,0),gao(k,1);
        if(cmp(a[1]/a[0],ans)==1) ans=a[1]/a[0],num=k;
    }
    printf("%d %.10f",num,ans);
    return 0;
}
